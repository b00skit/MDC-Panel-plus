...
type FormField = {
  type: 'text' | 'textarea' | 'dropdown' | 'officer' | 'general' | 'section' | 'hidden' | 'toggle' | 'datalist' | 'charge';
  name: string;
  label?: string;
  placeholder?: string;
  options?: string[];
      ...
      <form onSubmit={handleSubmit(onSubmit)} action={generatorConfig.formAction} method={generatorConfig.formMethod} className="space-y-6">
        {generatorConfig.form.map((field, index) => {
          switch (field.type) {
            ...
            case 'toggle':
                return (
                    <div key={`${field.name}-${index}`} className="flex items-center space-x-2">
                         <Controller
                            name={field.name}
                            control={control}
                            render={({ field: { onChange, value } }) => (
            ...
            case 'charge':
                return (
                    <div key={field.name + '-' + index} className="space-y-4">
                        {fields.map((chargeItem, chargeIndex) => (
