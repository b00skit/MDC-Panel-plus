...
export async function GET(
  ...
  let basePath = 'data';
  if(formType === 'user'){
    basePath = path.join(basePath, 'forms'); // Assuming custom stamps would be here
  } else {
    basePath = path.join(basePath, 'form-stamps');
    if(groupId) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Form Stamp not found' }, { status: 404 });
  }
}
